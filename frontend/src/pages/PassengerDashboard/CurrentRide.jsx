    ...
    <div className="mt-5">
      <div className="bg-white dark:text-gray-200 dark:bg-secondary-dark-bg p-6 rounded-2xl w-full md:w-1200 mb-4 mx-auto">
        ...
        <div className="flex justify-between items-center mt-5 border-t-1 border-color">
          ...
          <div className="mt-3">
            <Button color="white" bgColor={currentColor} text="Pay" borderRadius="10px" />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:text-gray-200 dark:bg-secondary-dark-bg p-6 rounded-2xl w-full md:w-1200 mb-4 mx-auto">
        <div className="flex justify-between items-center gap-2">
          <p className="text-xl font-semibold">Driver's Details</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full mt-6">
            <thead>
              <tr>
                ...
                <th className="text-left px-10 py-2">Vehicle License Number</th>
              </tr>
            </thead>
            <tbody>
              {dummyDrivers.map((driver, index) => (
                <tr key={driver.driverID}>
                  ...
                  <td className="text-left px-10 py-2">{driver.licenseNumber}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-white dark:text-gray-200 dark:bg-secondary-dark-bg p-6 rounded-2xl w-full md:w-1200 mb-4 mx-auto">
        <div className="flex justify-between items-center gap-2">
          <p className="text-xl font-semibold">Vehicle Details</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full mt-6">
            <thead>
            ...
            <tbody>
              {dummyVehicleData.map((vehicle, index) => (
                <tr
                  ...
                  className={index === dummyVehicleData.length - 1 ? '' : 'border-b border-gray-200'}
                >
                  <td className="text-left px-10 py-2">{vehicle.driverID}</td>
                  <td className="text-left px-10 py-2">{vehicle.vehicleType}</td>
                  <td className="text-left px-10 py-2">{vehicle.registrationNumber}</td>
                  <td className="text-left px-10 py-2">{vehicle.vehicleModel}</td>
                  <td className="text-left px-10 py-2">{vehicle.vehicleMake}</td>
                </tr>
              ))}
            </tbody>
          </table>
